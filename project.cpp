...
bool found = false;
    string line;
    ifstream file("Account_Detail.txt");
    ...
    while (getline(file, line)) {
        if (line.find("Account number: " + to_string(accountNumber)) != string::npos) {
            found = true;
            int balance;
            getline(file, line); // skip name
            getline(file, line); // skip account type
            getline(file, line); // read balance
            balance = stoi(line.substr(line.find(": ") + 2));
            balance += amount;
            temp << "Name: " << NameAccHolder << endl;
            temp << "Account Type: " << AccType << endl;
